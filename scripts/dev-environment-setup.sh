...
# PNPM is the package manager used in Theory's monorepo
install_pnpm () {
    PNPM_VERSION="9.0.6"
...
clone_monorepo () {
    ...
    if [[ -z "$SKIP" ]]; then
        echo ""
        echo "❓ Do you want to clone Theory's monorepo? ($POSITIVE_RESPONSE / $NEGATIVE_RESPONSE)"
        read -p " > " RESPONSE
	echo ""

    	if [[ "$RESPONSE" == "$POSITIVE_RESPONSE" ]]; then
            REPOSITORY="git@github.com:devartblake/TMT_Website.git"
            DESTINATION_FOLDER="$HOME/Dev"
            THEORY_FOLDER="$DESTINATION_FOLDER/theory"

            [[ ! -d "$DESTINATION_FOLDER" ]] && mkdir "$DESTINATION_FOLDER"
            if [[ ! -d "$THEORY_FOLDER" ]]; then
                git clone "$REPOSITORY $THEORY_FOLDER"
	        success_message "Theory monorepo"
            else
                already_installed_message "Theory monorepo"
            fi
        fi
    else
        skip_message "Theory monorepo"
        echo "$SKIP"
    fi
}

# Theory set of tools chosen
install_theory_tools () {
    check_git
    make_zsh_default_shell
    install_ohmyzsh
