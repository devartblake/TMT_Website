...
async function setupRunner() {
  ...
  inquirer.prompt(questions).then(async (answers) => {
    if (answers.action === DEV_ENVIRONMENT_SETUP) {
      ...
      try {
        shell.exec('nx run-many --target=build --projects=@tmt/api,@tmt/worker,@tmt/ws');
      ...
      shell.exec('npm run nx build @tmt/web');
...
function showWelcomeScreen() {
  ...
  console.log(chalk.bold(`                        Hi, I'm Gazoo by Theoretical Minds Technology!`));
  console.log(chalk.bold(textGradient(`  Welcome to the codebase of the open-source inventory management infrastructure\n`)));
