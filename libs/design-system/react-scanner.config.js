...
const Theory_ICON_REGEX = /^Icon(?!Button)[A-Z0-9]{1}[a-zA-Z0-9]+$/;
...
module.exports = {
   ...
   * Currently includes: theory, antd, mantine, and local imports
   *
   * To see only local imports, replace with: /(\.(\.){0,}\/.*)/gim
   */
  importedFrom:
    /(@theory\/(design-system|shared-web|notification-center)|@mantine\/core|@ant-design)(\/[a-z0-9\-)]+){0,}|(\.(\.){0,}\/.*)/gim,
...
function getComponentName({ imported, local, moduleName, importType }) {
  ...
  // get the module namespace / org (AKA Theory or mantine), but remove @
  const moduleOrg = moduleName.split('/').join('_').replace('@', '');

  // group Icons if from Theory Design System or AntD
  const name =
    (moduleName === '@theory/design-system' && Theory_ICON_REGEX.test(importedName)) || moduleName === ANTD_ICON_MODULE_NAME
      ? 'Icon'
      : importedName;
